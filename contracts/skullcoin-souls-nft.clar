...
;; Storage
(define-map token-count principal uint) ;; these are my NFT holders?
...
(define-public (transfer (id uint) (sender principal) (recipient principal))
  (begin
    ...
    (asserts! (is-none (map-get? market id)) ERR-LISTING) ;; cool don't try to send it if it's listed!
...
(define-public (set-mint-limit (limit uint))
  (begin
    (asserts! (is-eq tx-sender CONTRACT-OWNER) ERR-NOT-AUTHORIZED)
    (asserts! (< (var-get mint-limit) limit) ERR-MINT-LIMIT) ;; I can only extend the mint-limit
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      (asserts! (called-from-mint) ERR-NOT-AUTHORIZED) ;; okay you can only mint from the mint contract
      (asserts! (< (var-get last-id) (var-get mint-limit)) ERR-SOLD-OUT) ;; where does he set the mint-limit for the first time, upstaris u2000
      (match (nft-mint? skullcoin-souls next-id new-owner) 
        success ;; That's the value nft-mint? spits out and then what the code branch does
        (let
        ((current-balance (get-balance new-owner)))
          (begin
            ...
            (ok true)))
        error (err (* error u10002))))) ;; cool way to handle the different errors from nft-mint?
